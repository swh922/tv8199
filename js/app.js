...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {},
            `8199影视 - 免费在线视频搜索与观看平台`,
            `/`
        );
        // 更新页面标题
        document.title = `8199影视 - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - 8199影视`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== '8199影视-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = '8199影视-Settings';  // 配置文件名，用于校验
